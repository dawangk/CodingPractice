...
const int MM = 8e3+5; int mod = 1e9+7;//99824435

int n, k;ll dist[MM]; bool v[MM];
...
void prim(){
    ...
    for(int i = 0;i<n;i++){
        int mini = 0;
        for(int i = 1;i<=n;i++){
            if(!v[i]&&(mini==0||dist[i]<dist[mini]))mini = i;
        }v[mini] = true;
        for(int j = 1;j<=n;j++){
            if(!v[j])dist[j] = min(dist[j], (2019201913LL*min(mini,j)+2019201949LL*max(mini,j)) % 2019201997LL);
        }
    }
}
