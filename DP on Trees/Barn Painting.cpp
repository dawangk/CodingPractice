...
void dfs(int cur, int pre){
    for(int x: adj[cur]){
        if(x==pre)continue;
        dfs(x, cur);
        dp[cur][1] = mult(dp[cur][1], add(dp[x][2], dp[x][3]));
        dp[cur][2] = mult(dp[cur][2], add(dp[x][1], dp[x][3]));
        dp[cur][3] = mult(dp[cur][3], add(dp[x][2], dp[x][1]));
    }
}
...
int main(){
    ...
    dfs(1, -1);
    cout<<add(add(dp[1][1], dp[1][2]), dp[1][3])<<ell;
